...
function addToCart(productitem) {
    let parentElement = productitem.parents(".ss__product-item");
    let price = parseInt($.trim(parentElement.find(".singleProduct__cost--usd").text()));
    let size = $.trim(parentElement.find(".ss-store__variation--selected").text());
    let name = $.trim(parentElement.find(".ss-store__category").text());
...
$("#toOrderSubmit").on("click", function () {
    ...
    for (var i = 0; i < count.length; i++) {
        openConnection(hub, parseInt(count[i]), 3);
    }

    function openConnection(hub, storeId, notificationCount) {
        updaterConnection = new signalR.HubConnection(hub);
        updaterConnection
            .start()
            .then(() => {
                updaterConnection.invoke("NewOrders", storeId, notificationCount);
                // updaterConnection.invoke("Order", storeId, notificationCount);
            })
            .catch(() => {
