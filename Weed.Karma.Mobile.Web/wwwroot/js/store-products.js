...
$(".product-category-menu-item-link").on("click", function () {
    $('.product-category-menu-item-link').removeClass("active");
    $(this).addClass("active");
});
$('.size-item input').change(function () {
    let parent = $(this).parents(".product-item");
    let sizeItemCoast=parent.find(".size-item input:checked").val();

    parent.find(".product-price-coast").text(sizeItemCoast).attr('data-price-coast',sizeItemCoast);
});
$(".product-order-button").on("click",function () {
    addToCart($(this));
    $(this).css("background-color", "#014612");
    setTimeout(()=>{$(this).css("background-color", "#038423")},300)
});
